    ...
    <div className="relative">
      <div className="overflow-hidden" onClick={() => router.push(`/posts/${current.slug}`)}>
        <div className={`relative w-full aspect-video transition-opacity duration-300 ${animating ? "opacity-0" : "opacity-100"}`}>
          {current.imageUrl ? <Image src={`/api/notion-image?pageId=${current.id}`} alt={current.title} fill className="object-cover" /> : <div className="w-full h-full bg-gradient-to-br from-[#FCBC2A] to-[#092040]" />}
...
function HeroSlider({ posts }: { posts: Post[] }) {
  ...
  return (
    <div className="relative w-full aspect-video overflow-hidden cursor-pointer"
      ...
      {current.imageUrl ? <Image src={`/api/notion-image?pageId=${current.id}`} alt={current.title} fill className="object-cover" /> : <div className="w-full h-full bg-gray-100" />}
